...
void SocketHandleThread::stop()
{
	...
	{
		stopped = true;
		emit pushMsg(QString::fromLocal8Bit("数据队列中只有一个数据包正在发送，待该数据包发送完毕后，管程进入准备关闭状态"), id);
	}
	else if (!sendingInfo.sendingData.empty())
	{
		emit pushMsg(QString::fromLocal8Bit("数据队列中有数据包待发送，管程进入准备关闭状态失败"), id);
	}
	else
	{
		stopped = true;
		...
		case Public::ThreadState::Receieving:
			emit pushMsg(QString::fromLocal8Bit("数据队列为空，待该数据包接收完毕后，管程进入准备关闭状态"), id);
			break;
		case Public::ThreadState::Idle:
			emit pushMsg(QString::fromLocal8Bit("数据队列为空，且目前为空闲状态，管程进入准备关闭状态"), id);
			break;
		default:
			break;
		}
...
{
	emit pushMsg(QString::fromLocal8Bit("管程进入运行状态，将等待数据包的收发请求。"), id);
	...
	while (threadState == Public::ThreadState::Idle && !stopped)
	{
		if (threadState == Public::ThreadState::Idle)
		{
			...
			{
				// 转至准备发送态并发送SYN信号
				emit pushMsg(QString::fromLocal8Bit("管程目前为空闲状态，并有数据包待发送，将向目标发起发送数据包请求，并转入等待发送状态"), id);
...
void SocketHandleThread::dataReceivedForWaitSending(const Public::DataFrame &currFrame, Public::State frameState)
{
	if (currFrame.request == Public::RequestTypes::ACK)
	{
		...
		{
			// 初始化发送槽，并转入发送状态
			emit pushMsg(QString::fromLocal8Bit("已收到ACK(-1)信号，转入发送数据包状态"), id);
